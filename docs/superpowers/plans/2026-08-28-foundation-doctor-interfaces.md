...
entry point и автоматические проверки выполнены в окружении `.venv` с кодом
завершения 0; отсутствие Wireshark отображается диагностическим статусом без
traceback. Этот документ и README фиксируют результат в коммите
`Документировать первый этап разработки`.
